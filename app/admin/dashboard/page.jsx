...
const Reports = () => {
    ...
    const stats = useMemo(() => {
        ...
        const totalTurnoverTime = orders.reduce((total, order) => {
            if (!order.startTime || !order.endTime) return total;
            const startTime = new Date(order.startTime);
            const endTime = new Date(order.endTime);
            return total + (endTime - startTime);
        }, 0);
        const averageTurnoverMinutes = orders.length ? totalTurnoverTime / orders.length / 60000 : 0;
