...
export default function Reports() {
    ...
    const stats = useMemo(() => {
        ...
        const averageTableTimeMinutes = tableSessions.length ?
            totalTableTime / tableSessions.length / 60000 : 0;
        ...
        const comparisonToLastMonth = lastMonthSales === 0 ? 
            100 : ((monthlySales - lastMonthSales) / lastMonthSales) * 100;
            
        const comparisonToLastYear = lastYearSales === 0 ? 
            100 : ((yearlySales - lastYearSales) / lastYearSales) * 100;
            
        // Calculate average sales per day in current year
        const daysSoFarThisYear = Math.ceil((today - startOfYear) / (1000 * 60 * 60 * 24));
        const averagePerDay = daysSoFarThisYear > 0 ? yearlySales / daysSoFarThisYear : 0;
            ...
            <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="mb-6">
                    ...
                    <div className="flex items-center mt-2">
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getTrendBadgeClass(stats.comparisonToLastMonth)}`}>
                            {stats.comparisonToLastMonth >= 0 ? '↑' : '↓'} {Math.abs(stats.comparisonToLastMonth).toFixed(1)}%
                        </span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-gray-900">{formatCurrency(stats.dailySales)}</div>
                                        </div>
                                    ...
                                    <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-gray-900">{formatCurrency(stats.weeklyTotal)}</div>
                                        </div>
                                    ...
                                    <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-gray-900">{formatCurrency(stats.monthlySales)}</div>
                                            <div className={`text-xs font-medium ${getTrendBadgeClass(stats.comparisonToLastMonth)}`}>
                                                {stats.comparisonToLastMonth >= 0 ? '↑' : '↓'} {Math.abs(stats.comparisonToLastMonth).toFixed(1)}% from last month
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex justify-between items-center">
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-gray-900">{formatCurrency(stats.yearlyTotal)}</div>
                                            <div className={`text-xs font-medium ${getTrendBadgeClass(stats.comparisonToLastYear)}`}>
                                                {stats.comparisonToLastYear >= 0 ? '↑' : '↓'} {Math.abs(stats.comparisonToLastYear).toFixed(1)}% from last year
                                            </div>
                                        </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center border-b border-gray-100 pb-3">
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-gray-900">{stats.todayOrderCount}</div>
                                        </div>
                                    ...
                                    <div className="flex justify-between items-center">
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-gray-900">${stats.averageOrder.toFixed(2)}</div>
                                        </div>
                        ...
                        <div className="bg-gradient-to-br from-indigo-500 to-blue-600 rounded-2xl shadow-sm overflow-hidden text-white">
                            <div className="p-6">
                                <h2 className="text-lg font-semibold mb-4">Business Pattern Insights</h2>
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center border-b border-white/10 pb-3">
                                        <div>
                                            <div className="text-sm font-medium text-white/70">Busiest Day</div>
                                            <div className="text-xl font-bold">{stats.busyDay.day}</div>
                                            <div className="text-sm text-white/70">${stats.busyDay.amount.toFixed(2)} avg</div>
                                        </div>
                                    ...
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <div className="text-sm font-medium text-white/70">Slowest Day</div>
                                            <div className="text-xl font-bold">{stats.slowDay.day}</div>
                                            <div className="text-sm text-white/70">${stats.slowDay.amount.toFixed(2)} avg</div>
                                        </div>
                                        <div className="h-10 w-10 rounded-full bg-white/20 flex items-center justify-center">
                                            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl shadow-sm overflow-hidden p-6 border border-emerald-100">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <div className="text-sm font-medium text-emerald-600">Daily Avg Revenue</div>
                                        <div className="text-2xl font-bold text-gray-900 mt-2">${stats.averagePerDay.toFixed(2)}</div>
                                    </div>
                            ...
                            <div className="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-2xl shadow-sm overflow-hidden p-6 border border-amber-100">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <div className="text-sm font-medium text-amber-600">Avg Table Time</div>
                                        <div className="text-2xl font-bold text-gray-900 mt-2">{stats.averageTurnoverTime.toFixed(0)} min</div>
                                    </div>
                            ...
                            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl shadow-sm overflow-hidden p-6 border border-blue-100">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <div className="text-sm font-medium text-blue-600">Today&apos;s Sales</div>
                                        <div className="text-2xl font-bold text-gray-900 mt-2">${stats.dailySales.toFixed(2)}</div>
                                    </div>
