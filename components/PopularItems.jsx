...
const PopularItems = ({ orders, menuItems }) => {
    ...
    return Object.entries(itemStats)
      .map(([itemId, stats]) => {
        const menuItem = flatMenuItems.find(item => 
          item.id === parseInt(itemId) || item.menu_item_id === parseInt(itemId)
        );
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm">
      <h2 className="text-lg font-semibold mb-4 text-black">Popular Items</h2>
      <div className="space-y-4">
        {popularItems.map((item, index) => (
          <div key={index} className="flex items-center justify-between">
            ...
            <span className="font-medium text-black">${item.revenue.toFixed(2)}</span>
          </div>
        ))}
      </div>
    </div>
