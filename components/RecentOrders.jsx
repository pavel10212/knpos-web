          ...
          <tbody className="divide-y divide-gray-200">
            {orders.map((order) => (
              <tr key={order.order_id}>
                ...
                <td className="px-6 py-4 text-sm text-gray-900">{order.order_details.length}</td>
                <td className="px-6 py-4 text-sm text-gray-900">${order.total_amount.toFixed(2)}</td>
                <td className="px-6 py-4 text-sm">
                  <span className={`px-2 py-1 rounded-full text-xs ${getOrderStatusClass(order.order_status)}`}>
                    {order.order_status}
                  </span>
                </td>
              </tr>
